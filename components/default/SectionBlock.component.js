...
function SectionBlock({ activeSession, router, routerList }) {
  return (
    <section
      className={`${
        activeSession ? 'col-span-2' : 'col-span-1'
      } h-full border-b xl:border-0`}
    >
      {/* wrapper */}
      <div className='h-section-left sticky top-8'>
        ...
        <div className='h-section-left flex flex-col justify-between m-8'>
          ...
          {/* navigation footer */}
          <div className='flex justify-end space-x-8 py-4 xl:py-0'>
            <div>github</div>
            <div>about</div>
          </div>
        </div>
